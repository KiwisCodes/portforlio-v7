...
export function Projects() {
  ...
  return (
    <section id="projects" className="py-24 px-6 lg:px-12 max-w-7xl mx-auto relative">
      ...
      <div className="w-full py-12 max-w-[320px] sm:max-w-[400px] md:max-w-[500px] lg:max-w-[600px] mx-auto relative group/swiper">
        ...
        >
          {PROJECTS.map((project, idx) => (
            <SwiperSlide key={project.id} className="rounded-2xl border border-border bg-bg-secondary overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.5)]">
              <div 
              ...
              />
              <div className={`absolute inset-0 z-0 bg-gradient-to-b ${project.color} opacity-40 transition-opacity duration-500`} />
              
              <div className={`relative z-10 flex flex-col h-full`}>
                <div className="p-8 flex-1 flex flex-col items-start translate-z-10" style={{ transform: 'translateZ(30px)' }}>
                  {project.featured && (
                  ...
                  <p className="font-mono text-xs text-text-tertiary mb-6 uppercase tracking-wider">{project.subtitle}</p>
                  <p className="text-text-secondary mb-8 max-w-sm line-clamp-3">{project.description}</p>
                  
                  <div className="mt-auto flex flex-wrap gap-2 mb-8">
                    {project.tags.slice(0, 3).map(tag => (
                      <span key={tag} className="px-3 py-1 rounded-md bg-bg-tertiary border border-border text-xs font-mono text-text-primary">
                        {tag}
                      </span>
                    ))}
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 py-12 md:p-12 overflow-y-auto">
            ...
            >
              ...
              <div className="p-8 md:p-12 md:w-1/2 flex flex-col items-start text-left relative z-10 bg-bg-secondary">
                <p className="font-mono text-xs text-accent-gold mb-2 uppercase tracking-wider">{selectedProject.subtitle}</p>
                <h3 className="font-display text-4xl mb-6">{selectedProject.title}</h3>
                
                <p className="text-text-secondary leading-relaxed mb-8">{selectedProject.description}</p>
                ...
                <div className="space-y-3 mb-10 w-full">
                  {selectedProject.details.map((detail, i) => (
                    <div key={i} className="flex flex-row items-baseline gap-3">
                      ...
                      <span className="text-sm text-text-primary leading-relaxed text-left">{detail}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-10">
                  {selectedProject.tags.map(tag => (
                    <span key={tag} className="px-3 py-1.5 rounded-md bg-bg-tertiary border border-border text-xs font-mono text-text-secondary">
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-auto flex gap-4 w-full">
                  <a href="#" className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-text-primary text-bg-primary font-medium hover:bg-text-primary/90 transition-colors">
                    <Github className="w-4 h-4" /> Code
                  </a>
                  <a href="#" className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border border-border hover:bg-bg-tertiary transition-colors">
